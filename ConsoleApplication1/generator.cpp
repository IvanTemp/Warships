...
void Output_Field() {
	...
	for (int y = 0; y < width_height; y++) {
		...
		for (int x = 0; x < width_height; x++) {
			std::cout << OutputMap[Field_ID[x][y].first] << "|";
		}
		std::cout << std::endl;
	}
}
		...
		case 2: //South
			if (y < width_height - length) {
				for (int h = 0; h < length; h++) {
					if (Field_ID[x][y + h].first != 0) breaksIn = false;
				}
				if (breaksIn) {
					if (y > 0) {
						if (x > 0 && Field_ID[x - 1][y - 1].first != 1) Field_ID[x - 1][y - 1].first = 3;
						Field_ID[x][y - 1].first = 3;
						if (x < width_height - 1 && Field_ID[x + 1][y - 1].first != 1) Field_ID[x + 1][y - 1].first = 3;
					}
					for (int temp_int = 0; temp_int < length; temp_int++) {
						if (x > 0 && Field_ID[x - 1][y + temp_int].first != 1) Field_ID[x - 1][y + temp_int].first = 4;
						Field_ID[x][y + temp_int].first = ID;
						if (x < width_height - 1 && Field_ID[x + 1][y + temp_int].first != 1) Field_ID[x + 1][y + temp_int].first = 5;
					}
					if (y < width_height - 1) {
						if (x > 0 && Field_ID[x - 1][y + length].first != 1) Field_ID[x - 1][y + length].first = 6;
						Field_ID[x][y + length].first = 6;
						if (x < width_height - 1 && Field_ID[x + 1][y + length].first != 1) Field_ID[x + 1][y + length].first = 6;
					}
					stop++;
				}
				else continue;
